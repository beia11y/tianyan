...
function cleanPath(path) {
  // 云函数 HTTP 触发路径可能包含函数名前缀，例如 /tianyan-api/api/chat
  return (path || '')
    .replace(/^\/tianyan-api/, '')
    .replace(/^\//, '');
}
...
exports.main = async (event, context) => {
  if (!DEEPSEEK_KEY) {
    ...
    return makeResponse({ error: 'missing DEEPSEEK_KEY' }, 500);
  }
  ...
  // 健康检查
  if (path === '' || path === '/') {
    return makeResponse({ ok: true, service: 'tianyan-api' });
  }

  // 角色对话代理
  if (path === 'api/chat' && method === 'POST') {
    try {
      const { speakerKey, speakerName, personality, context, userInput } = parseBody(event);
...
只能返回 JSON，不要任何其他内容。`;
      ...
      try {
        ...
        console.log('JSON parse failed, use raw output');
      }
  ...
  if (path === 'api/ending' && method === 'POST') {
    try {
      const { code, theme, profile } = parseBody(event);
